...
def load_config():
    ...
    if 'paths' not in config:
        ...
        config['paths']['template_weekly_path'] = \
            input('What is the path for the weekly template?')
        config['paths']['template_monthly_path'] = \
            input('What is the path for the monthly template?')
        config['paths']['template_semester_path'] = \
            input('What is the path for the semester template?')
        config['paths']['template_annual_path'] = \
            input('What is the path for the annual template?')
        bookmark_path = \
            os.path.join(os.path.expanduser('~'), '.config/gtk-3.0/bookmarks')
        ...
        if 'tickler_path' not in config['paths']:
            config['paths']['tickler_path'] = \
                input('What is the tickler-path?')
            config_changed = True
...
def check_weekly_rp_session(tickler_path,
                            activation_path,
                            nautilus_bookmark_path,
                            template_weekly_path):
    ...
    if not os.path.exists(filepathname):
        ...
        update_nautilus_bookmark(
            nautilus_bookmark_path, filepathname, 'next_weekly')
        print('created new weekly review: ' + filename)
    return


def check_monthly_rp_session(tickler_path,
                             activation_path,
                             nautilus_bookmark_path,
                             template_monthly_path):
    today = datetime.date.today()
    ...
    if not os.path.exists(filepathname):
        ...
        update_nautilus_bookmark(
            nautilus_bookmark_path, filepathname, 'next_monthly')
        print('created new monthly review: ' + filename)

    return


def check_semester_rp_session(tickler_path,
                              activation_path,
                              nautilus_bookmark_path,
                              template_semester_path):
    ...
    if not os.path.exists(filepathname):
        ...
        update_nautilus_bookmark(
            nautilus_bookmark_path, filepathname, 'next_semester')
        print('created new semester review: ' + filename)
    return


def check_annual_tasks(tickler_path,
                       activation_path,
                       nautilus_bookmark_path,
                       template_annual_path):
    current_year = datetime.datetime.now().year
...
def check_tickler(tickler_path, activation_path):
    ...
    for item in os.listdir(tickler_path):
        ...
        if current_year == item_year and \
                current_month == item_month and \
